...
describe('Checking if merkle root is working', function () {
  it('Should be able to verify if a given address is in whitelist or not', async function () {

    // bunch of test cases.
    const [owner, addr1, addr2, addr3, addr4, addr5] = await ethers.getSigners();

    //an array of elements you wish to encode in the Merkle Tree
    const list = [encodeLeaf(owner.address, 2), encodeLeaf(addr1.address, 2), encodeLeaf(addr2.address, 2), encodeLeaf(addr3.address, 2), encodeLeaf(addr4.address, 2), encodeLeaf(addr5.address, 2),];
